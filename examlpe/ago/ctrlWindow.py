...
class CtrlWindow(object):
    ...
    def initEvents(self):
        ...
        self.ui.east.clicked.connect(lambda: self.main.MoveThread.addLogic.emit([signalBin.Logic(2,self.nowPet,"toProcess",int(self.ui.nSteps.text()),"east",None)]))
        self.ui.west.clicked.connect(lambda: self.main.MoveThread.addLogic.emit([signalBin.Logic(2,self.nowPet,"toProcess",int(self.ui.nSteps.text()),"west",None)]))
        self.ui.north.clicked.connect(lambda: self.main.MoveThread.addLogic.emit([signalBin.Logic(2,self.nowPet,"toProcess",int(self.ui.nSteps.text()),"north",None)]))
        self.ui.south.clicked.connect(lambda: self.main.MoveThread.addLogic.emit([signalBin.Logic(2,self.nowPet,"toProcess",int(self.ui.nSteps.text()),"south",None)]))
        ...
        self.nowPet.changeStatusSignal.emit("stand")
    def randomItem(self):
        """
        从资源文件夹中随机获取一张图片
        """
